export default class FlightRestService {
    ...
    constructor() {
        this.flights = null;
        this.url = "http://localhost:8080/flight";
    }
    ...
    getFlightByCode(code) {
        return fetch(this.url + "/find/" + code)
        .then(response => {
            if (!response.ok) {
                this.handleResponseError(response);
            }
    ...
    getFlightByCarrier(carrier) {
        return fetch(this.url + "/carrier/" + carrier)
        .then(response => {
            if (!response.ok) {
                this.handleResponseError(response);
            }
    ...
    async getFlightByRoute(source, destination) {
        return await fetch(this.url + "/route" + "?source=" + source + "&destination=" + destination)
        .then(response => {
            if (!response.ok) {
                this.handleResponseError(response);
            }
    ...
    async getFlightByPriceRange(min, max) {
        return await fetch(this.url + "/price" + "?min=" + min + "&max=" + max)
        .then(response => {
            if (!response.ok) {
                this.handleResponseError(response);
            }
